...
**想要架设一个自己的服务器，那么就得先认识一下::material-symbols:terminal::运行环境。**

::: note
如果你对这篇教程中的服务器专有术语不太明白，你可以先看看下一节：[术语入门](/guide/ji-chu-zhi-shi-xue-xi/terminology/)

不过这篇教程也对一些术语做了详细的概述，小白可以放心阅读
:::
...
::: card title="软件层面" icon="eos-icons:software"
...
::: card title="硬件层面" icon="ion:hardware-chip"
...
::: card title="网络层面" icon="material-symbols:wifi-rounded"
- ::mdi:ip::公网IP地址
...
顾名思义，Java版Minecraft服务器是==用Java语言编写的程序==，因此必须在Java虚拟机（JVM）上运行。这就像一个翻译官，将Java代码翻译成计算机能够理解的指令。
...
一般只能运行Java程序，而且体积比较小（约80-150MB），**适合仅运行服务器的用户**
:::
::: card title="JDK（Java Development Kit，Java开发工具包）"
包含JRE的所有功能及一些额外包含开发工具，体积较大（约300-500MB），**适合需要编译插件或MOD的用户**
:::
::::
...
1. **Windows系统：**
   ...
   4. 双击运行安装程序, 一路“下一步”即可 [+注]

   [+注]: 安装路径可以自行决定

   方法二：OpenJDK <Badge type="important" text="免费开源" />
...
JVM参数控制Java虚拟机的运行方式，如果配置合理就可以显著提升服务器性能。
...
- `-Xms`：设置初始内存
- `-jar`：指定要运行的服务器文件
- `nogui`：不启动图形界面（节省资源）<Badge type="tip" text="推荐" />

::: tip
`Xmx` 和 `Xms` 设置为相同值可以减少内存分配的开销，提升性能
:::

**优化参数：**<Badge type="important" text="适用于Java 17+" />
...
有图形界面友好，易于操作，比较适合小白，插件和MOD兼容性也比较好，本地测试的方便性毋庸置疑

:::

::: card title="缺点" icon="icon-park-solid:bad-two"
资源占用较高（系统本身需要2-4GB内存），性能也不如Linux
:::
...
资源占用极低，性能优异，稳定性强，且免费开源，是云服务器的标配

:::

::: card title="缺点" icon="icon-park-solid:bad-two"

上手比较困难，需要一定技术基础，而且都是纯命令行操作
...
2. **性能设置：**
   - 调整为“最佳性能”模式（作用可能不是很大）
   - 关闭视觉效果
   - 禁用系统还原 <Badge type="info" text="可选" />

3. **防火墙配置：**
   - 允许Java通过防火墙
   - 开放服务器端口（MC服务器默认是`25565`）
...
| 小型整合包（轻量整合） | 8GB | 6GB | 1-10人，50个MOD以内 |
| 大型整合包（水槽包） | 16GB+ | 12GB+ | 1-20人，100+个MOD |
...
:::: card-grid
::: card title="HDD（机械硬盘）" icon="mdi:hdd"
- 读写速度：80-160MB/s
- 优点：容量大, 价格便宜
- 缺点：速度慢, 会造成卡顿

建议用于备份的存储，但是如果要拿来开服务器还是别想了
:::
::: card title="SSD（固态硬盘）" icon="solar:ssd-round-bold"
- 读写速度：500-3500MB/s
- 优点：速度快, 几乎无卡顿
- 缺点：价格较贵

服务器储存选择的标配，没有之一，**强烈推荐用于服务器主存储！**
:::
::: card title="NVMe SSD" icon="bi:nvme-fill"
- 读写速度：3000-7000MB/s
- 最佳选择，性能极致
:::
::::
...
端口映射就是把你==路由器的公网IP的某个端口==“转发”到你电脑的==内网IP的服务器端口==上，让外网玩家能够连接到你的服务器。
...
3. **步骤三：防火墙设置**
   ...
   **Windows防火墙：**
   ...
   5. 允许连接
   6. 全部配置文件勾选
   7. 命名为“Minecraft Server”（或者一个你自己喜欢的名称）
...
4. **步骤四：获取公网IP**
   ...
   ::: warning 家庭宽带的限制
   1. 很多地区运营商不再分配公网IP，这种情况需要使用内网穿透软件
   2. 重启路由器后IP会变（动态IP），需要使用DDNS服务
   :::

### 5.2 内网穿透方案

当你没有公网IP时，需要使用==内网穿透==软件：
...
域名比IP地址更容易记忆，例如我的服务器IP是 `123.456.789.012:25565`，现在我整了个域名解析，把这条IP解析到我的域名之后，连接到服务器的IP就变成了这样：`play.myserver.com`，是不是更容易记住且更美观？
...
服务器核心就是Minecraft服务器的“大脑”，不同的核心有不同的特性和用途。
...
- 需要使用BuildTools编译 <Badge type="warning" text="复杂" />
...
3. **原因三：过多的实体**
   ...
   ```
      ...
      ::: warning
      建议在清理前提前通知一声，让服务器里的玩家知情，~~否则你的祖宗十八代将迎来一次点兵点将~~

   你也可以安装优化相关的插件自动清理，如ClearLagg。
...
1. 使用G1GC垃圾回收器 <Badge type="important" text="Java 17+" />:
...
#### 问题1：“Connection refused（拒绝链接）”或“无法连接服务器”
...
::: warning
如果你的服务器跟别人闹矛盾了，就把 `online-mode` 设为 `true`（开启正版验证），否则就会存在一些风险……
...
1. **定期自动备份**
   ...
   安装备份插件，可以是 SimpleBackup、BackupManager、WorldEdit（手动备份）一类的插件

2. **使用脚本的自动备份 <Badge type="info" text="Linux" />**
...
::: card title="环境准备" icon="fluent-mdl2:server-enviroment"
- [ ] 安装正确版本的Java
- [ ] 配置Java环境变量
- [ ] 选择合适的操作系统
- [ ] 确保硬件满足要求
:::
::: card title="服务器配置" icon="mynaui:config"
- [ ] 下载合适的服务器核心
- [ ] 创建启动脚本
- [ ] 同意EULA协议
- [ ] 配置server.properties
:::
::: card title="网络配置" icon="iconoir:internet"
- [ ] 设置端口映射 <Badge type="info" text="家庭宽带" />
- [ ] 或使用内网穿透
- [ ] 或直接使用云服务器
- [ ] 配置防火墙规则
:::
::: card title="安全和维护" icon="wpf:maintenance"
- [ ] 设置白名单或权限插件
- [ ] 配置自动备份
- [ ] 安装必要的管理插件
- [ ] 制定服务器规则
...
:::: card-grid
::: card title="新手测试服（预算0元）" icon="tabler:letter-a"
- 本机Windows系统
- 2-4GB内存分配
- Paper核心
- 樱花Frp内网穿透
- 适合: 3-5人朋友服
:::
::: card title="小型服务器（预算50-100元/月）" icon="tabler:letter-b"
- 云服务器 2核4GB
- Ubuntu 22.04系统
- Paper核心 + 基础插件
- 5-10Mbps带宽
- 适合: 10-30人社区服
:::
::: card title="中型服务器（预算200-500元/月）" icon="tabler:letter-c"
- 云服务器 4核8GB
- Ubuntu 22.04系统
- Paper/Purpur核心 + 丰富插件
- 10-30Mbps带宽
- 适合: 30-100人公开服
:::
:::card title="大型服务器（预算1000+元/月）" icon="tabler:letter-d"
- 独立服务器 8核16GB+
- Linux系统
- 多服务器集群(BungeeCord)
- 100Mbps+带宽
...
搭建和运营一个服务器是一个==持续学习==的过程。虽然本教程涵盖了搭建服务器的所有基础知识，但实际运营中你会遇到各种各样的新问题和麻烦……

记住了，重要的事情我只说一遍：

1. **不要**一开始就追求大型服务器，可以先开一个几个朋友玩的小服务器来开始积累经验
2. **要**不断学习，因为技术一直在不断更新，保持学习新知识的态度
3. **要**养成备份的好习惯，定期备份是保护数据的唯一方法
4. **要**多搜索和多提问（QQ群和vx群不是摆设），遇到问题多查阅文档，
5. **要**以玩家为核心，技术要服务于体验，始终以玩家体验为中心
